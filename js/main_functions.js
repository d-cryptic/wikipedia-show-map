...
function renameNode(oldId, newName) {
  ...
  if (nodes.get(newId)) {
    ...
    nodes.add({
      ...oldNode,
      id: newId,
      label: wordwrap(newName, oldNode.level === 0 ? 20 : 15),
    });
  }
  ...
  window.tracenodes = window.tracenodes.map((id) =>
    id === oldId ? newId : id
  );
  // If the node was a start node, replace it
  window.startpages = window.startpages.map((id) =>
    id === oldId ? newId : id
  );
  // Return the new ID
  return newId;
...
function expandNodeCallback(page, data) {
  ...
  for (let i = 0; i < subpages.length; i += 1) {
    ...
    if (!getEdgeConnecting(page, subpageID)) {
      // Don't create duplicate edges in same direction
      newedges.push({
        from: page,
        to: subpageID,
...
function getTraceBackNodes(node) {
  ...
  const path = [];
  while (!finished) {
    // Add parents of nodes until we reach the start
    path.push(currentNode);
    if (window.startpages.indexOf(currentNode) !== -1) {
...
function getTraceBackEdges(tbnodes) {
  tbnodes.reverse();
  const path = [];
  for (let i = 0; i < tbnodes.length - 1; i += 1) {
    // Don't iterate through the last node
