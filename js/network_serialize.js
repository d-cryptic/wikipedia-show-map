...
function abbreviateNode(node) {
  ...
  const newnode = { a: node.label, b: node.level, c: node.parent };
  return newnode;
}
...
function abbreviateEdge(edge) {
  ...
  */
  const newedge = { a: edge.from, b: edge.to, c: edge.level };
  return newedge;
}
...
function unabbreviateEdge(edge) {
  const newedge = { from: edge.a, to: edge.b, level: edge.c };
...
function howConcise() {
  ...
  const unAbbreviatedLength =
    JSON.stringify(nodes._data).length +
  ...
  console.log(
    `Abbreviation takes JSON size from ${unAbbreviatedLength} bytes (unabbreviated) to ${abbreviatedLength} bytes (abbreviated)`
  );
