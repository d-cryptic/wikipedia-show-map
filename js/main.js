...
function setStartPages(starts) {
  ...
  const toRemove = window.startpages.filter(
    (id) => !newStartPages.includes(id)
  );
  const toAdd = starts.filter(
    (pageName, i) => !window.startpages.includes(newStartPages[i])
  );
...
function go() {
  ...
  Promise.all(inputs.map(fetchPageTitle)).then((pageTitles) => {
    // Record on the commafield item which node the input corresponds to
    pageTitles.forEach((pageTitle, i) => {
      cf.getElementsByClassName("item")[i].dataset.nodeId =
        getNormalizedId(pageTitle);
    });
