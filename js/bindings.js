...
function expandEvent(params) {
  // Expand a node (with event handler)
...
function mobileTraceEvent(params) {
  // Trace back a node (with event handler)
  if (params.nodes.length) {
    // Was the click on a node?
    // The node clicked
...
function bindNetwork() {
  if (isTouchDevice) {
    ...
    // Device does not have touchscreen
...
function bind() {
  ...
  ghbutton.addEventListener("click", () =>
    window.open("https://github.com/controversial/wikipedia-map", "_blank")
  );
  ...
  aboutButton.addEventListener("click", () =>
    window.open(
      "https://github.com/controversial/wikipedia-map/blob/master/README.md#usage",
      "_blank"
    )
  );
}
